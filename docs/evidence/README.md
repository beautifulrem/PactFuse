...
- `/api/v1/evidence/proof-bundle`: the operator-only `PACTFUSE_PUBLIC_PROOF_BUNDLE_V1` export for the latest proof-authorized public claim; it binds the public claim, claim-input replay bundle, verifier run, provider status, deployment registry, server metadata, and bundle hash.
