...
- Artifact preflight / Judge Check / claim readiness / runner heartbeat / CAW receipt ingest / Agent Transcript / replay bundle / public proof bundle records are API-backed. Replay bundles include indexed page roots and embedded page proofs, but checked-in public evidence remains pending until live Cobo identity, deployed token, and external-chain receipts are captured.
...
- `/api/v1/evidence/proof-bundle`: the operator-only `PACTFUSE_PUBLIC_PROOF_BUNDLE_V1` export for the latest proof-authorized public claim; it binds the public claim hash, public-claim event hash, claim-input replay bundle, verifier run hash, provider status hash, deployment registry hash, server metadata hash, and proof bundle hash. Provider status, deployment registry, and server metadata are captured in the authorization event and reused on read. `server.generatedAt` is fixed to the proof authorization event timestamp, not the read time.
- Mock-token public claims require a live deployment registry entry binding the payment token address to chain id, non-zero deployment transaction hash, public HTTPS explorer URL, decimals, and non-zero code hash. Official-USDC public claims require chain id `84532`, a passed official-USDC probe, and a matching live registry entry.
