...
AI Agent 开始自己花钱了(买数据、模型、工具、算力),用的是像 **Cobo Agentic Wallet (CAW)** 这样的钱包。真正麻烦的是 **LLM Agent 怎么判断"这东西能不能买"**,而这个判断在四个方面都很脆:

1. **静态知识滞后:** 模型不知道最新的 exploit、CVE、制裁、脱锚、失效的价格源。
2. **搜索结果滞后:** Web 搜索可能搜到旧文章、SEO 垃圾、转载站、缓存页。
3. **引用不可靠:** LLM 会编造链接、错误归因、把二手来源当一手。
4. **读到信息与执行之间有时间差:** Agent 10:00 读到"这个来源安全",10:05 准备转钱,但 10:03 它已被攻击或被风控标记。

最深的是第 4 条,一个典型的 TOCTOU(检查时间 / 使用时间不一致)漏洞:LLM 把一条搜索结果、一个引用、一段网页摘要,当成了关于某个来源的**可执行事实**,可它只是一张随时可能已经过时的快照。这正是今天真金白银在损失的地方:

- 预言机价格刚被操纵,机器人就照着它执行;
- 供应商银行账号在发票审批到打款之间被掉包(BEC 诈骗);
- 某个依赖包或模型刚被拉进来,就被标记为恶意(CVE、后门)。

再搜一遍救不了它:问题不在搜索次数,而在**资金要动的那一刻有没有一道强制的状态检查**。只在决定时检查一次远远不够。**必须在钱要动的那一刻再查一次,有变化就拒绝。**
...
<div align="center">
<img src="docs/assets/console-demo.gif" alt="PactFuse Console 跑三个风险场景:支付在付款前熔断、结算并交付、或被策略拒绝" width="860">
</div>
...
任何"基于一个会过期的事实去付款"的自主执行者都需要这道闸,而 LLM Agent 最需要:它是从可能已经过时的搜索结果和引用里做决定的。三个具体形态:

- **Agent 自动把 treasury 投进 DeFi。** 它读到"这个 vault 审计过、APY 高、口碑好",但那可能是缓存文章、旧审计或 SEO 软文。把这笔 deposit 绑定到 vault / oracle / bridge 来源;若监控方已对它发起挑战(exploit 警报、审计撤销、TVL 异常流出、admin key 变更),deposit 在资金转移前就熔断。
- **Agent 自动采购 API、模型或数据集。** vendor"看起来不错",但付款前它可能刚泄露、模型被标恶意、包被投毒、许可证被撤销。把这笔 spend 绑定到 artifact 发行方背书;来源被吊销或被挑战就拒付。
- **Agent 根据新闻自动交易。** 那条"利好"可能是假消息、被盗账号、转载旧闻,或钓鱼合约地址。把交易绑定到 token / 合约来源 + 白名单目标;来源被风控挑战、或目标不在白名单,就永远不会结算。

**是执行,不是检测。** PactFuse 自己不扫 CVE、不盯 TVL。检测方(CVE / OSV 源、风控、审计方、预言机健康 watcher)在链上发起挑战;PactFuse 保证"被挑战的来源付不出去"。检测可插拔,执行是这道闸。同一套原语也服务非 AI 执行者:交易 bot、跨链执行器、treasury 流转。

> 搜索结果不是结算级事实,LLM 引用不是动钱许可。

与本仓库 live demo 最贴合的是采购那一类:Agent 购买一份来源绑定的工具租约,而来源可能在付款前被挑战。
