...
**为什么 AI Agent 尤其需要。** LLM Agent 只是最容易"拿过期或伪造的事实去执行"的那种执行者:搜索差、知识滞后、会编造引用、会被提示注入。PactFuse 把 LLM 的"看起来安全"从一种**许可**降级为一个**意图**,并在资金要动的那一刻重新核验来源。检测保持可插拔(CVE 源、风控、审计方、预言机健康负责发起挑战);执行被保证(来源被挑战 → 不付款)。*搜索结果不是结算级事实,LLM 引用不是动钱许可。*
