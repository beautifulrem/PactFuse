...
**为什么 AI Agent 尤其需要。** 在所有执行者里,LLM Agent 最容易拿上面那些过期或伪造的事实去执行,所以最需要这道闸。PactFuse 把 LLM 的"看起来安全"从一种**许可**降级为一个**意图**,在资金要动的那一刻重新核验:检测保持可插拔(CVE 源、风控、审计方、预言机健康负责发起挑战),执行被保证(来源被挑战 → 不付款)。*搜索结果不是结算级事实,LLM 引用不是动钱许可。*
