...
const STR = {
  en: {
    ...
    "sc.trip.s1d": "challenge evidence enters the session ledger",
    "sc.trip.s2d": "the indexer confirms the public-chain risk event",
    "sc.trip.s3d": "the breaker opens, cutting the payment path for every spend pinned to that source",
    "sc.settle.s1d": "ERC20 approval signed inside the Pact policy boundary",
    "sc.settle.s2d": "allowance {a} → {b} at block {n}",
    "sc.settle.s3d": "SpendSettled finalized · market balance {a} → {b}",
    "sc.settle.s4d": "bearer-bound access token; MCP transcript pinned to the bought artifact",
    "sc.deny.s1d": "target address is not in the Pact allowlist",
    "sc.deny.s2d": "CAW evaluates target, selector, and limits server-side",
    "sc.deny.s3d": "persisted as live_denied audit evidence; no transaction exists",
    "fail.reason": "transport drop before the action completed, retry clears it",
    ...
    "auth.proof": "proof", "auth.delivery": "delivery", "jc.pass": "pass", "jc.evidence": "evidence", "jc.viewTx": "view tx", "jc.offchain": "off-chain · no on-chain tx",
    ...
    "hash.attestKey": "attestation key", "hash.note": "note", "hash.contracts": "on-chain contracts", "hash.token": "payment token",
    ...
    "drawer.chainState": "Live on-chain state", "drawer.chainStateSub": "read live from the deployed contracts (keyless)",
    "link.chainState": "live state", "cs.sessionH": "this session, live",
    "cs.reading": "reading…", "cs.rpcError": "RPC unreachable", "cs.none": "no on-chain state in fixture mode",
    "cs.src.unknown": "Unknown", "cs.src.active": "Active", "cs.src.challenged": "Challenged", "cs.src.revoked": "Revoked",
    "cs.spend.unknown": "Unknown", "cs.spend.registered": "Registered", "cs.spend.tripped": "Tripped", "cs.spend.settled": "Settled",
    "cs.probeH": "Check any source freshness", "cs.probeBtn": "read state", "cs.invalid": "enter a 0x… 32-byte source hash",

    "help.title": "Keyboard shortcuts",
    "help.r1": "Select risk scenario", "help.r2": "Run / retry the scenario",
    "help.r3": "Clear evidence log", "help.r4": "Reset to idle",
    "help.r5": "Judge check · Proof hashes · Self-test · Live state", "help.r6": "Toggle this help", "help.r7": "Close",
  ...
  zh: {
    ...
    "head.proof3b": "回执自证", "head.proof3s": "页面每条声明都重放链上签名证据,{n} 项评审校验通过", "head.proof3sFixture": "fixture 模式:证据不可用,无已验证通过",
    "chip.verified": "已验证证据", "chip.fixture": "fixture 回退",
    "chip.liveClaim": "实时声明 · mock-ERC20 结算(测试网)",
    "link.judge": "评审校验 {p}/{t}", "link.judgeEmpty": "评审校验",
    ...
    "metric.ledgerSeq": "账本序号", "metric.cawAudit": "CAW 审计行",

    "panel.scenarios": "风险场景", "panel.scenariosHint": "重放已验证的证据",
    "btn.run": "运行场景", "btn.running": "证据重放中", "btn.retry": "重试",
    "btn.runAgain": "再次运行", "btn.reset": "复位", "toggle.fail": "模拟传输中断",

    "sc.trip.title": "不安全来源 → 自动熔断",
    "sc.trip.lede": "钉住的来源在报价后变得不安全。闸门必须在资金转移前切断付款。",
    "sc.trip.outcome": "已保护 · 受挑战来源上 0 资金转移",
    ...
    "sc.settle.outcome": "已结算 · 转移 {n} {sym}-atomic",
    ...
    "sc.trip.s1d": "挑战证据进入本会话账本",
    "sc.trip.s2d": "索引器确认这条公链风险事件",
    "sc.trip.s3d": "断路器跳开,切断每一笔钉住该来源的支付回路",
    "sc.settle.s1d": "在 Pact 策略边界内签署 ERC20 授权",
    "sc.settle.s2d": "额度 {a} → {b} @ 区块 {n}",
    "sc.settle.s3d": "SpendSettled 已最终确认 · 市场余额 {a} → {b}",
    "sc.settle.s4d": "bearer 绑定的访问令牌;MCP 调用记录钉定到所购 artifact",
    "sc.deny.s1d": "目标地址不在 Pact 白名单内",
    "sc.deny.s2d": "CAW 在服务端评估目标、选择器与限额",
    "sc.deny.s3d": "作为 live_denied 审计证据持久化;不存在任何交易",
    "fail.reason": "传输在动作完成前中断,重试即可清除",

    "stage.event": "事件", "stage.detect": "检测", "stage.respond": "响应", "stage.done": "完成",
    "note.idle": "选择一个场景开始", "note.armed": "已就绪 · 可重放证据",
    ...
    "auth.proof": "证明", "auth.delivery": "交付", "jc.pass": "通过", "jc.evidence": "证据", "jc.viewTx": "查看交易", "jc.offchain": "链下 · 无链上交易",
    ...
    "hash.attestKey": "签名公钥", "hash.note": "说明", "hash.contracts": "链上合约", "hash.token": "支付代币",
    "st.loaded": "证据包已加载", "st.verified": "来源:已验证重放",
    "st.fixture": "来源:fixture 回退", "st.judge": "评审校验 {p}/{t}",
    ...
    "drawer.chainState": "链上实时状态", "drawer.chainStateSub": "从已部署合约实时读取(无需密钥)",
    "link.chainState": "实时状态", "cs.sessionH": "本会话 · 实时",
    "cs.reading": "读取中…", "cs.rpcError": "RPC 无法访问", "cs.none": "fixture 模式无链上状态",
    "cs.src.unknown": "未知", "cs.src.active": "活跃", "cs.src.challenged": "已挑战", "cs.src.revoked": "已吊销",
    "cs.spend.unknown": "未知", "cs.spend.registered": "已登记", "cs.spend.tripped": "已熔断", "cs.spend.settled": "已结算",
    "cs.probeH": "自查任意来源的新鲜度", "cs.probeBtn": "读取状态", "cs.invalid": "请输入 0x 开头的 32 字节来源哈希",

    "help.title": "键盘快捷键",
    "help.r1": "选择风险场景", "help.r2": "运行 / 重试场景",
    "help.r3": "清空证据日志", "help.r4": "复位到空闲",
    "help.r5": "评审校验 · 证明哈希 · 自检 · 实时状态", "help.r6": "切换本帮助", "help.r7": "关闭",
    ...
    "toast.session": "已复制会话 ID", "toast.hash": "已复制哈希", "toast.copyFail": "复制失败",
  },
};
